...
static struct jl_lh_table *json_object_table;

static void json_object_init(void) __attribute__ ((constructor));
static void json_object_init(void) {
  MC_DEBUG("json_object_init: creating object table\n");
  json_object_table = jl_lh_kptr_table_new(128, "json_object_table", NULL);
}
...
static void json_object_fini(void) {
  ...
  if(MC_GET_DEBUG()) {
    if (json_object_table->count) {
      ...
      jl_lh_foreach(json_object_table, ent) {
  ...
  jl_lh_table_free(json_object_table);
}
#endif /* REFCOUNT_DEBUG */
...
static int json_escape_str(struct jl_printbuf *pb, char *str)
{
...
static void json_object_generic_delete(struct json_object* jso)
{
  ...
  jl_lh_table_delete(json_object_table, jso);
#endif /* REFCOUNT_DEBUG */
...
#ifdef REFCOUNT_DEBUG
  jl_lh_table_insert(json_object_table, jso, jso);
...
static void json_object_lh_entry_free(struct jl_lh_entry *ent)
{
  free(ent->k);
...
struct jl_lh_table* json_object_get_object(struct json_object *jso)
{
  if(!jso) return NULL;
  switch(jso->o_type) {
...
{
  return (struct json_object*) jl_lh_table_lookup(jso->o.c_object, key);
}
...
struct json_object* json_object_new_array(void)
{
  ...
  jso->o.c_array = jl_array_list_new(&json_object_array_entry_free);
  return jso;
}

struct jl_array_list* json_object_get_array(struct json_object *jso)
{
  if(!jso) return NULL;
  switch(jso->o_type) {
...
struct json_object* json_object_array_get_idx(struct json_object *jso,
					      int idx)
{
  return (struct json_object*)jl_array_list_get_idx(jso->o.c_array, idx);
}
