    ...
    for(int j = 0; implicit_testmatches[i].queries[j].query != NULL; j++) {
      ...
      if(ast) {
        if(benchmark) {
          ...
          implicit_testmatches[i].queries[j].bench_ns = mtev_perftimer_elapsed(&btimer);
        } else {
          match = noit_metric_tag_search_evaluate_against_tags(ast, &tagset);
        }
        test_assert_namef(match == implicit_testmatches[i].queries[j].match, "'%s' %s", implicit_testmatches[i].queries[j].query, match ? "matches" : "doesn't match");
        /* clone if only to test cloning */
        noit_metric_tag_search_free(noit_metric_tag_search_clone(ast));
        noit_metric_tag_search_free(ast);
      } else {
        test_assert_namef(ast != NULL, "parsing error at %d in '%s'", erroroffset, implicit_testmatches[i].queries[j].query);
      }
    }
    free(tagset.tags);
    // free(canonical);
