    ...
    select nspname||'.'||relname as tablename
     ...
     where relkind = 'r'
       and ( ( relname like 'metric_numeric_archive_%' and relname <= ('metric_numeric_archive_' || v_tdate) ) or
             ( relname like 'metric_text_archive_%' and relname <= ('metric_text_archive_' || v_tdate) ) or
             ( relname like 'check_archive_%' and relname <= ('check_archive_' || v_tdate) ) or
             ( relname like 'check_status_archive_%' and relname <= ('check_status_archive_' || v_tdate) ) )
     order by relname 
  loop
